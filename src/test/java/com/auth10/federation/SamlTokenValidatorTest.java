...
public class SamlTokenValidatorTest extends TestCase {
	...
	private FederatedConfiguration config;
	
	public SamlTokenValidatorTest(String name) throws Exception {
		super(name);
		config = FederatedConfiguration.getInstance("src/test/resources/federation.properties");
	}

	public void testOrgIdCertificateShouldBeValid() throws Exception {

		config.setThumbprint("3F5DFCDF4B3D0EAB9BA49BEFB3CFD760DA9CCCF1");
		config.setAudienceURI("spn:19b8831d-0827-432f-bfb6-e587e48c3ea9@014310f7-b77f-44f7-8e06-2943a117ea20");

		SamlTokenValidator validator = new SamlTokenValidator();
		validator.setValidateExpiration(false);

		List<Claim> claims = validator.validate(office365Token).getClaims();
	...
	public void testShouldValidateSaml11TokenWithRSTRAndReturnClaims()
		...
		config.setTrustedIssuers("CN=SelfSTS");
		config.setAudienceURI("https://localhost/javafederationtest/");
		SamlTokenValidator validator = new SamlTokenValidator();
		validator.setValidateExpiration(false);

		List<Claim> claims = validator.validate(validTestToken).getClaims();
	...
	public void testShouldThrowBecauseTheTokenWasIssuedByUntrustedIssuer()
		...
		config.setTrustedIssuers("");
		config.setThumbprint("");
		validator.setValidateExpiration(false);
		try {
	...
	public void testShouldThrowBecauseTheTokenAppliesToUnknownAudience()
		...
		config.setThumbprint(invalidTrustedIssuer);
